...
void turn_backward_PID(uint32_t dist)
{
	...
	int count1A = __HAL_TIM_GET_COUNTER(&htim2);
	int count1B = __HAL_TIM_GET_COUNTER(&htim3);
...
void encoder(void *argument)
{
  ...
  uint32_t count1A, count1B, cnt2A, cnt2B, diff1A, diff1B, diff2A, diff2B, dur, tick1, tick2;
  uint8_t yawer[10];
  		float gz;
  		float tmnow = 0;
  		float tmold = 0;
  ...
  count1A = __HAL_TIM_GET_COUNTER(&htim2);
  count1B = __HAL_TIM_GET_COUNTER(&htim3);
...
//	  			 HAL_UART_Transmit(&huart3, yawer, 10, HAL_MAX_DELAY);
		...
		diff1A = abs(count1A - cnt2A);
		diff1B = abs(count1B - cnt2B);
		diff2A = abs(65535 - (__HAL_TIM_IS_TIM_COUNTING_DOWN(&htim2) ? cnt2A - count1A : count1A - cnt2A));
		diff2B = abs(65535 - (__HAL_TIM_IS_TIM_COUNTING_DOWN(&htim3) ? cnt2B - count1B : count1B - cnt2B));
		...
		distA += 1.065* encGrad * pi * radius * encoderAVal / 165;
		distB += encGrad * pi * radius * encoderBVal / 165;
...
int main(void)
{
  ...
  yaw_g = 0;
  tempA = 0;
  adjustDist = 5; //adjustDist > distBuffer + encInt
  changeDist = 10;
  encDelay = 25;
  lumos_dir = 'N';
  encGrad = 0.204234324563945;
  encInt = 1.084353464531;
